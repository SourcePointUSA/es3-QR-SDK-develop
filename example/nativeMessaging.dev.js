(function () {
	...
	(function () {
	  ...
	  var triggerEvent = function triggerEvent(eventName, args) {
	  ...
	  function extendSpObject() {
	    var executeMessagingFunc = function executeMessagingFunc(targeting) {
	    ...
	    var loadPrivacyManagerModalFunc = function loadPrivacyManagerModalFunc() {
	      updateQrUrl(getQrCodeUrl());
	      showElement(pmDiv);
	      hideElement(messageDiv);
	    };
	    var acceptAllFunc = function acceptAllFunc() {
	      hideElement(pmDiv);
	      hideElement(messageDiv);
	      acceptAll();
	    };
	    ...
	    var consentStatusFunc = function consentStatusFunc() {
	      return consentStatus;
	    };
	    var getTcStringFunc = function getTcStringFunc() {
	      return euConsentString;
	    };
	    var getQrCodeUrlFunc = function getQrCodeUrlFunc() {
	      return getQrCodeUrl();
	    };
	    var getMessageDataFunc = function getMessageDataFunc() {
	      return messageCategoryData;
	    };
	    var clearUserDataFunc = function clearUserDataFunc() {
	      deleteCookie("authId");
	    ...
	    var updateConsentStatusFunc = function updateConsentStatusFunc() {
	      hideElement(pmDiv);
	      hideElement(messageDiv);
	      getConsentStatus();
	      getMessages();
	  ...
	  function getMessages() {
	    if (shouldCallMessagesEndpoint()) {
	      ...
	      var fullURL = (isJSONp ? proxyEndpoint + "/jsonp" : baseEndpoint) + baseURL + queryParams + '&body=' + json2QueryParam(body) + '&localState=' + json2QueryParam(localState) + '&metadata=' + json2QueryParam(metaData) + '&nonKeyedLocalState=' + json2QueryParam(nonKeyedLocalState) + '&ch=' + cb + '&scriptVersion=' + scriptVersion + '&scriptType=' + scriptType;
	      if (isJSONp) {
	        ...
	        setItem("nonKeyedLocalState_" + propertyId, JSON.parse(res.nonKeyedLocalState));
	  ...
	  function useJsonpPostRequest(endpoint, data, callbackName) {
	    ...
	    var fullUrl = base + "?body=" + encodeURIComponent(JSON.stringify(data)) + "&" + toQueryParams(params);
	    ...
	    if (isJSONp) {
	      ...
	      req.onreadystatechange = function () {
	        ...
	        if (req.status === 200) {
	          ...
	          storeConsentResponse(res.consentStatus, res.uuid, res.dateCreated, res.euconsent, res.grants, res.categories, res.vendors, res.legIntVendors, res.legIntCategories);
	        } else {
	  ...
	  function sendAcceptAllRequest(consentdata) {
	    ...
	    if (isJSONp) {
	      ...
	      req.onreadystatechange = function () {
	        ...
	        if (req.status === 200) {
	          var res = JSON.parse(req.responseText);
	          storeConsentResponse(consentdata.gdpr.consentStatus, res.uuid, res.dateCreated, euConsentString, consentdata.gdpr.grants, consentdata.gdpr.categories, consentdata.gdpr.vendors, consentdata.gdpr.legIntVendors, consentdata.gdpr.legIntCategories);
	        } else {
	          onError('error:', req);
	        }
	      };
	  ...
	  function storeConsentResponse(conStatus, uuid, cDate, euconsent, vGrants, purposes, vendors, legIntV, legIntP) {
	    ...
	    setItem("consentStatus_" + propertyId, conStatus);
	    consentUUID = uuid;
	    ...
	    setItem("euconsent-v2_" + propertyId, euconsent);
	    setItem("vendorGrants_" + propertyId, vGrants);
	    vendorGrants = vGrants;
	    setItem("acceptedCategories_" + propertyId, purposes);
	    acceptedCategories = purposes;
	    setItem("acceptedVendors_" + propertyId, vendors);
	    acceptedVendors = vendors;
	    setItem("legIntCategories_" + propertyId, legIntP);
	    legIntCategories = legIntP;
	    setItem("legIntVendors_" + propertyId, legIntV);
	  ...
	  function getConsentStatus(forced) {
	    ...
	    if (isJSONp) {
	      ...
	      if (typeof res.consentStatusData.gdpr !== undefined) {
	        hasLocalData = true;
	        storeConsentResponse(res.consentStatusData.gdpr.consentStatus, res.consentStatusData.gdpr.consentUUID, res.consentStatusData.gdpr.dateCreated, res.consentStatusData.gdpr.euconsent, res.consentStatusData.gdpr.grants, res.consentStatusData.gdpr.acceptedVendors, res.consentStatusData.gdpr.legIntVendors, res.consentStatusData.gdpr.legIntCategories);
	      }
	    }
	  }
	  ...
	  function getQrCodeUrl() {
	    ...
	    return _sp_.config.qrUrl + encodeURIComponent(_sp_.config.pmUrl + "?authid=" + authId + "&consentlanguage=" + consentLanguage + "&propertyid=" + propertyId + "&propertyhref=" + propertyHref + "&accountid=" + accountId + "&pmid=" + pmId + additionalParams);
	  }
	  ...
	  function buildMessage() {
	    if (buildMessageComponents) {
	      ...
	      for (var j = 0; j < data.categories.length; j++) {
	        ...
	        newHTML = newHTML.replace("{name}", category.name || "").replace("{description}", category.description || "");
	  ...
	  function getMetaData() {
	    ...
	    if (isJSONp) {
	      ...
	      setItem("metaData_" + propertyId, metaData);
	    }
	  }
	  ...
	  function handleGetMessagesForJsonP(data) {
	    ...
	    setItem("localState_" + propertyId, JSON.parse(data.localState));
	    nonKeyedLocalState = data.nonKeyedLocalState;
	    setItem("nonKeyedLocalState_" + propertyId, JSON.parse(data.nonKeyedLocalState));
	  ...
	  function handleGetConsentStatusForJsonP(data) {
	    if (typeof data.consentStatusData.gdpr !== undefined) {
	      hasLocalData = true;
	      storeConsentResponse(data.consentStatusData.gdpr.consentStatus, data.consentStatusData.gdpr.uuid, data.consentStatusData.gdpr.dateCreated, data.consentStatusData.gdpr.euconsent, data.consentStatusData.gdpr.grants, data.consentStatusData.gdpr.acceptedCategories, data.consentStatusData.gdpr.acceptedVendors, data.consentStatusData.gdpr.legIntVendors, data.consentStatusData.gdpr.legIntCategories);
	    }
	  }
	  ...
	  function handleJsonpAcceptAllCall(data) {
	    if (data) {
	      storeConsentResponse(consentStatus, data.uuid, data.dateCreated, euConsentString, vendorGrants, acceptedCategories, acceptedVendors, legIntVendors, legIntCategories);
	    } else {
	  ...
	  function handleLiOnlyCall(data) {
	    storeConsentResponse(data.consentStatus, data.uuid, data.dateCreated, data.euconsent, data.grants, data.acceptedCategories, data.acceptedVendors, data.legIntVendors, data.legIntCategories);
	  }
	  ...
	  function isReadyToExecute(_temp) {
	    var _ref = _temp === void 0 ? {} : _temp,
	      ...
	      startTime = _ref$startTime === void 0 ? Date.now() : _ref$startTime;
...
//# sourceMappingURL=nativeMessaging.dev.js.map
