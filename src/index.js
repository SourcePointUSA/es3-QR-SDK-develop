...
(function () {
  ...
  function extendSpObject() {
    ...
    var deletePropertyScopedDataForProperty = function (targetPropertyId) {
      if (targetPropertyId === null || typeof targetPropertyId === "undefined") {
        return;
      }
    ...
    var collectPropertyScopedKeys = function (propertyIds) {
      ...
      var collectKey = function (key) {
        ...
        for (var idx = 0; idx < propertyScopedPrefixes.length; idx++) {
          ...
          if (!isNaN(suffixId) && (allowAll || propertyIdLookup[suffixId])) {
            keys[key] = true;
          }
          return;
        }
      ...
      if (typeof window.localStorage !== "undefined") {
        try {
          for (var idx = 0; idx < window.localStorage.length; idx++) {
            collectKey(window.localStorage.key(idx));
          }
        } catch (e) {}
      }
    ...
    var clearPropertyScopedData = function (options) {
      if (options && options.clearAllPropertyScopedData === true) {
        ...
        for (var idx = 0; idx < propertyScopedKeys.length; idx++) {
          deleteStorageKeyEverywhere(propertyScopedKeys[idx]);
        }
        return;
      }
